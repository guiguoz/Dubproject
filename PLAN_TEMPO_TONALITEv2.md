...
## C2. Détection de tonalité au chargement + key-match CONDITIONNEL

**Fichiers :** `KeyDetector`/`KeyResult` existants, `ScenePreloader`,
`MusicContext.keyRoot`, UI slot.

**RÈGLE FONDAMENTALE (invariant à respecter dans tout le code de cette section) :**
le key-match ne transpose JAMAIS un sample par défaut. Une transposition
automatique n'est appliquée que si TOUTES les conditions suivantes sont vraies :
1. l'utilisateur a défini MANUELLEMENT la tonalité projet
   (`MusicContext.keyRoot` ≠ −1, saisie via l'UI — jamais déduite/devinée par
   l'app ; tant qu'aucune tonalité n'est saisie, keyRoot = −1 et AUCUN key-match
   n'est possible, nulle part) ;
2. la tonalité détectée du sample est connue avec une confiance suffisante
   (seuil ~0.7) ;
3. la tonalité détectée NE CORRESPOND PAS à la tonalité projet — « correspond »
   = même tonique, ou relative majeure/mineure (Am ≡ C), ou écart 0 demi-ton ;
   dans tous ces cas : transposeRatio reste STRICTEMENT 1.0, aucun traitement,
   PCM intact ;
4. le key-match est activé pour ce slot (toggle par slot, ET l'option globale
   « Key-match » est ON — off par défaut) ;
5. le rôle du slot est MELODIC/BASS/PAD — les KICK/PERC/FX ne sont JAMAIS
   key-matchés, quelle que soit la configuration.

En dehors de ce chemin, la seule transposition existante est celle demandée
explicitement par l'utilisateur via le contrôle C1.

Implémentation :
- À l'import d'un sample MELODIC/BASS/PAD : KeyDetector sur le PCM original en
  tâche de fond ; stocker `detectedKey` + confiance dans les métadonnées du slot
  (et dans la scène). La détection est purement INFORMATIVE : elle n'entraîne
  aucune action seule.
- Badge UI sur le slot : « Am » — vert si conforme à la tonalité projet (aucune
  action possible ni nécessaire), orange si écart (« Am · −2 st »), gris « ? » si
  confiance faible ou si aucune tonalité projet n'est définie.
- Badge orange + clic = PROPOSER la transposition via C1 (plus court chemin :
  min(|delta|, 12−|delta|), signe choisi pour rester dans ±7 st ; au-delà,
  proposer la relative min/maj). C'est le mode normal : suggestion, action
  utilisateur.
- Le mode automatique (conditions 1–5 réunies) applique cette même transposition
  sans clic, au chargement du sample ou lors d'un changement manuel de la
  tonalité projet. Tout transpose auto est visible et annulable : le champ
  transpose du slot (C1) affiche la valeur appliquée, remise à 0 possible à tout
  moment.
- Changement de la tonalité projet par l'utilisateur → recalcul : les slots
  redevenus conformes repassent à transposeRatio = 1.0 (retour au PCM original,
  pas de re-traitement en cascade — cf. A3).

**Tests :**
- T-C2a : fixtures de loops de tonalité connue (progressions synthétiques en
  C maj, A min, F# min) → détection correcte ≥ 90 %, suggestion = plus court
  chemin.
- T-C2b (invariant) : sample détecté Am, tonalité projet = Am ou C maj, tous les
  toggles ON → transposeRatio == 1.0 exactement, buffer de lecture ==
  PCM stretché original (aucun pitch-shift déclenché, vérifier qu'aucune tâche
  WSOLA n'est lancée).
- T-C2c : tonalité projet NON définie (keyRoot = −1), toggles ON → aucun
  transpose appliqué sur aucun slot, badges gris.
- T-C2d : slot rôle KICK, tonalité projet définie, toggles ON → jamais transposé.
- T-C2e : projet en C, sample en D (auto-transpose −2 st appliqué), l'utilisateur
  passe le projet en D → le slot revient à transposeRatio 1.0 depuis l'original.
...
Chaque scène stocke sa tonalité, saisie MANUELLEMENT par l'utilisateur (champ
éditable ; par défaut : vide = pas de tonalité, donc aucun key-match dans cette
scène). La règle fondamentale de C2 s'applique intégralement ici : à la
transition, pour chaque slot commun (KEEP), on compare la tonalité DÉTECTÉE du
sample à la tonalité de la scène cible ; un événement de transpose quantisé à
launchStep (pipeline C1) n'est émis QUE si le sample n'est pas conforme (mêmes
critères de conformité que C2 : tonique identique ou relative = conforme →
transposeRatio 1.0, aucun événement, aucun traitement) ET si le key-match est
actif pour ce slot. Un slot déjà dans la tonalité de la scène cible traverse la
transition sans aucune modification. Les slots ENTER appliquent leur transpose
stocké par scène (valeur explicite, saisie ou validée par l'utilisateur — jamais
recalculée en douce). Si la scène cible n'a pas de tonalité définie : aucun
transpose n'est émis, les valeurs de transpose stockées par slot sont simplement
restaurées telles quelles.
Dépendance : PLAN_REFONTE_TRANSITIONS §3 (catégorie KEEP). Sans la refonte,
implémenter la version simple : même logique conditionnelle, appliquée dans
applyScene.
**Test T-C3 :** scène A (tonalité Am) → scène B (tonalité Cm) avec 3 slots KEEP :
un sample détecté Cm (conforme cible), un détecté Am (écart), un KICK →
exactement UN événement de transpose émis (le sample Am), le sample Cm et le
kick traversent avec transposeRatio 1.0.
