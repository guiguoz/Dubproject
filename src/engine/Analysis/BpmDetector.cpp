...
// ─────────────────────────────────────────────────────────────────────────────
// detectWithOctaveCorrection
// ─────────────────────────────────────────────────────────────────────────────
BpmDetector::BpmDetectionResult
BpmDetector::detectWithOctaveCorrection(const float* pcm, int numFrames,
                                         float sampleRate, float projectBpm) noexcept
{
    const auto raw = detectOfflineRobust(pcm, numFrames, static_cast<double>(sampleRate));

    if (projectBpm <= 0.0f)
        return raw; // pas de correction demandée

    const float bpm    = raw.bpm;
    const float double_ = bpm * 2.0f;
    const float half_   = bpm * 0.5f;

    // Distances absolues vis-à-vis du projectBpm
    const float distBase   = std::abs(bpm    - projectBpm);
    const float distDouble = std::abs(double_ - projectBpm);
    const float distHalf   = std::abs(half_   - projectBpm);

    float corrected = bpm;
    if (distDouble <= distBase && distHalf >= distDouble)
    {
        // double candidate
        if (double_ < 300.0f)
            corrected = double_;
    }
    else if (distHalf <= distBase)
    {
        // half candidate
        if (half_ > 40.0f)
            corrected = half_;
    ...
    return {corrected, raw.confidence};
}
