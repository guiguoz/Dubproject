...
class BpmDetector
{
  public:
    ...
    // ── Octave correction ─────────────────────────────────────────────────────
    /// Détecte le BPM brut puis choisit parmi {bpm, bpm×2, bpm÷2} le candidat
    /// le plus proche du BPM projet.
    /// - bpm×2 retenu si bpm×2 < 300 et plus proche de projectBpm
    /// - bpm÷2 retenu si bpm÷2 > 40  et plus proche de projectBpm
    /// Si projectBpm <= 0, retourne le résultat brut sans correction.
    static BpmDetectionResult detectWithOctaveCorrection(const float* pcm,
                                                         int numFrames,
                                                         float sampleRate,
