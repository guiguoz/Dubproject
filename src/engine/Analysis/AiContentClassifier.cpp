...
std::vector<float> AiContentClassifier::padOrCrop(const std::vector<float>& samples,
                                                    size_t targetSize)
{
...
std::vector<float> AiContentClassifier::extractFeatures(const std::vector<float>& pcm)
{
    ...
    // Attack time (samples to reach peak within first 100ms)
    size_t peakIdx = 0;
    float  peakVal = 0.0f;
    const size_t attackLookback = std::min(n, static_cast<size_t>(4410));
    ...
    const float attackRatio = static_cast<float>(peakIdx) / 44100.0f;
    ...
    std::vector<float> bandEnergy(bands.size() + 1, 0.0f);
...
static std::vector<float> runInference(AiContentClassifier& self,
                                        ...
                                        double sampleRate)
{
    ...
    std::vector<float> padded   = self.padOrCrop(resampledPcm, targetLen);
    std::vector<float> features = self.extractFeatures(padded);
    self.normalizeFeatures(features);

    std::vector<float> logits;
    try { logits = self.model_.run(features); }
    catch (const std::exception& e) {
        std::fprintf(stderr, "AiContentClassifier: inference failed: %s\n", e.what());
    }
    catch (...) {
        std::fprintf(stderr, "AiContentClassifier: inference failed (unknown)\n");
    }
    return logits;
}

static AiContentClassifier::ContentType indexToType(size_t idx) noexcept
{
    switch (idx) {
        case 0: return AiContentClassifier::ContentType::KICK;
        case 1: return AiContentClassifier::ContentType::SNARE;
        case 2: return AiContentClassifier::ContentType::HIHAT;
        case 3: return AiContentClassifier::ContentType::BASS;
        case 4: return AiContentClassifier::ContentType::SYNTH;
        case 5: return AiContentClassifier::ContentType::PAD;
        case 6: return AiContentClassifier::ContentType::PERC;
        default: return AiContentClassifier::ContentType::OTHER;
    }
}

AiContentClassifier::ContentType
AiContentClassifier::classify(const std::vector<float>& pcm, double sampleRate)
{
    return classifyWithConfidence(pcm, sampleRate).type;
}

AiContentClassifier::ClassifyResult
AiContentClassifier::classifyWithConfidence(const std::vector<float>& pcm,
                                             double sampleRate)
{
    const auto logits = runInference(*this, pcm, sampleRate);
    if (logits.empty()) return {ContentType::OTHER, 0.0f};
    ...
    float sumExp = 0.0f;
    for (float l : logits) sumExp += std::exp(l - maxVal);
    const float confidence = 1.0f / sumExp; // exp(0) / sumExp

    return {indexToType(bestIdx), confidence};
}
